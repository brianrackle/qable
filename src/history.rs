...
impl History {
    ...
    pub fn get_records_index(&self, imdb_ids: &[String], predicate: impl Fn(&Record) -> bool) -> Vec<Option<usize>> {
        imdb_ids.iter().map(|imdb_id| self.get_record_index(imdb_id, &predicate)).collect()
    }
    ...
    pub fn get_record(&self, imdb_id: &String, predicate: impl Fn(&Record) -> bool) -> Option<&Record> {
        ...
        for record in self.records.iter() {
            if record.imdb_id == *imdb_id {
                if predicate(record) { found = Some(record); } else { break; }
            }
        }
        found
    }

    pub fn get_record_index(&self, imdb_id: &String, predicate: impl Fn(&Record) -> bool) -> Option<usize> {
        let mut found = None;
        for (i, record) in self.records.iter().enumerate() {
            if record.imdb_id == *imdb_id {
                if predicate(record) { found = Some(i); } else { break; }
            }
        }
        found
    }
...
impl MediaManager {
    ...
    pub fn clean_library(&mut self, save: bool) {
        ...
        for missing_id in missing_ids {
            let position = self.history.get_record_index(&missing_id, |_| true).unwrap();
        ...
        for record in self.history.records.iter()
            ...
            if let Some(tmdb_title) = tmdb::get_movie_title(&self.config, &record.imdb_id) {
                ...
                if record.title != tmdb_title {
                    if let Some(key) = plex::find_key_by_imdb_id(&self.config, &record.imdb_id) {
